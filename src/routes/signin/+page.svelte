<script>
  import Input from '$lib/input.svelte'
  import { signIn } from "@auth/sveltekit/client"
</script>

<main class="grid grid-rows-[0.5fr_1fr] min-h-screen">
   <div class="self-center grid gap-4">
     <h1 class="text-center text-2xl text-gray-600">Bem vindo de volta</h1>
     <p class="text-center text-2xl text-gray-600">Entre na sua conta</p>
   </div>
   
   <form class="grid p-16">
     <div class="grid gap-6 self-start">
       <Input placeholder="Usuário" type="text" />
       <Input placeholder="Senha" type="password" />
       <div class="flex items-center justify-between">
         <Input type="checkbox" label="Lembrar-me" />
         <div class="text-sm">
           <a href="#" class="font-medium text-red-700 underline hover:text-red-700">Esqueceu a senha?</a>
         </div>
       </div>
     </div>

      <div class="self-end">
       <div>
          <button on:click={signIn} class="w-full px-4 py-2 text-white bg-red-600 border border-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500" />
       </div>
       <p class="mt-2 text-center text-sm text-gray-600">
         Não tem uma conta? &nbsp <a href="signup" class="font-medium underline text-red-700 hover:text-red-500">Cadastrar</a>
       </p>
     </div>
   </form>
 </main>
